  ...
  return (
    <main className="min-h-screen bg-[#07101D] text-white">
      <div className="mx-auto max-w-[1540px] px-4 py-4 md:px-6 md:py-5">
        <header className="mb-4 flex flex-col gap-3 rounded-xl border border-white/[0.08] bg-[#0C1626] px-4 py-3 shadow-[0_20px_50px_rgba(0,0,0,0.18)] lg:flex-row lg:items-center lg:justify-between">
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-blue-600 text-sm font-bold shadow-lg shadow-blue-950/40">MP</div>
            <div>
          ...
          <nav className="flex flex-wrap items-center gap-2">
            <span className="mr-1 rounded-md border border-white/[0.07] bg-[#08111F] px-3 py-2 text-[11px] text-slate-500">
              Son güncelleme <strong className="ml-1 font-medium text-slate-300">{lastUpdated}</strong>
            </span>
        ...
        <section className="mb-4 grid gap-3 xl:grid-cols-[1.15fr_0.85fr]">
          <div className="rounded-xl border border-white/[0.08] bg-[linear-gradient(135deg,#0E1A2C_0%,#0A1322_100%)] p-5 shadow-[0_24px_60px_rgba(0,0,0,0.18)]">
            <div className="max-w-2xl">
              ...
              <p className="mt-2 max-w-xl text-sm leading-6 text-slate-400">Zincir marketlerdeki seçili ürünlerin son durumu, yeni fiyat hareketleri ve erişim sorunları tek ekranda.</p>
            </div>
          ...
          <div className="overflow-hidden rounded-xl border border-white/[0.08] bg-[#0C1626] shadow-[0_20px_50px_rgba(0,0,0,0.16)]">
            <div className="flex items-center justify-between border-b border-white/[0.07] px-4 py-3">
              <div>
                <h2 className="text-sm font-semibold text-slate-100">Market durumu</h2>
                <p className="text-[10px] text-slate-600">Son kontrol sonuçları</p>
              </div>
                  ...
                  <div className="flex min-w-0 items-center gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="text-[10px] text-slate-600">
                        {summary.changedToday > 0 ? `${summary.changedToday} üründe bugün fiyat değişti` : summary.unreadable > 0 ? `${summary.unreadable} ürün okunamadı` : "Yeni fiyat değişimi yok"}
                      </div>
                    </div>
                  </div>
        ...
        <section className="overflow-hidden rounded-xl border border-white/[0.08] bg-[#0C1626] shadow-[0_20px_50px_rgba(0,0,0,0.16)]">
          <div className="flex items-center justify-between border-b border-white/[0.07] px-4 py-3">
            <div>
              <h2 className="text-sm font-semibold text-slate-100">Bugünkü fiyat hareketleri</h2>
              <p className="text-[10px] text-slate-600">Yalnızca bugün kaydedilen yeni değişiklikler</p>
            </div>
          ...
          {todayChangedItems.length > 0 ? (
            <div className="divide-y divide-white/[0.055]">
              {todayChangedItems.slice(0, 5).map((item) => (
                <Link key={`${item.market}-${item.sku}`} href={`/report/detail?sku=${encodeURIComponent(item.sku)}`} className="grid grid-cols-[1fr_auto_auto] items-center gap-3 px-4 py-2.5 text-sm transition hover:bg-white/[0.025]">
                  <div className="min-w-0">
                    <div className="truncate text-xs font-medium text-slate-200">{item.name}</div>
            ...
            <div className="flex min-h-[118px] items-center justify-center px-4 text-center">
              <div>
                <div className="mx-auto mb-2 flex h-8 w-8 items-center justify-center rounded-full border border-emerald-400/15 bg-emerald-400/[0.06] text-emerald-300">✓</div>
                <p className="text-sm font-medium text-slate-300">Bugün yeni fiyat değişikliği yok</p>
                <p className="mt-1 text-[11px] text-slate-600">Son kontrol tamamlandı, yeni hareket tespit edilmedi.</p>
              </div>
            </div>
          )}
        </section>
