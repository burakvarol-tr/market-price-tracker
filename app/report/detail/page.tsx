...
export default async function ProductDetailPage({ searchParams }: { searchParams: Promise<{ sku?: string }> }) {
  ...
  const chartData = pricedHistory.slice(0, 30).reverse().map((item) => ({ date: item.checkedAt, price: item.price as number }));
    ...
    <main className="min-h-screen bg-[#07101D] text-white">
      <div className="mx-auto max-w-[1540px] px-4 py-4 md:px-6 md:py-5">
        <section className="mb-4 overflow-hidden rounded-[20px] border border-white/[0.09] bg-[radial-gradient(circle_at_top_right,rgba(37,99,235,0.18),transparent_34%),linear-gradient(135deg,#0E1A2C_0%,#091321_100%)] shadow-[0_24px_65px_rgba(0,0,0,0.2)]">
          <div className={`grid items-stretch ${latest.imageUrl ? "lg:grid-cols-[1fr_210px]" : ""}`}>
            ...
            {latest.imageUrl && (
              <div className="flex items-center justify-center border-t border-white/[0.08] bg-white/[0.018] p-4 lg:border-l lg:border-t-0">
                <SafeProductImage
                  ...
                  imageClassName={`h-full w-full object-contain ${latest.market === "CARREFOUR" ? "scale-[1.28]" : "scale-105"}`}
                />
              </div>
            )}
          </div>
        ...
        {unreadable && (
          <section className="mb-4 flex items-start gap-3 rounded-xl border border-amber-400/20 bg-amber-500/[0.055] px-4 py-3 text-sm text-amber-100">
            <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full border border-amber-300/25 text-[11px]">!</span>
            <span>Fiyat ve stok bilgisi şu anda okunamadı. Bu durum ürünün stokta olmadığı anlamına gelmez.</span>
          </section>
        )}
        ...
        <section className="mb-4 grid gap-3 md:grid-cols-3">
          {[["Güncel fiyat", formatPrice(latest.currentPrice)], ["Önceki fiyat", formatPrice(previousPrice)], ["Durum", statusText]].map(([label, value]) => (
            <div key={String(label)} className="rounded-[16px] border border-white/[0.085] bg-[linear-gradient(180deg,rgba(255,255,255,0.035),rgba(255,255,255,0.018))] px-4 py-3.5 shadow-[0_14px_35px_rgba(0,0,0,0.13)]">
              <div className="text-[11px] uppercase tracking-[0.08em] text-slate-500">{label}</div>
              <div className="mt-1.5 break-words text-xl font-semibold tracking-[-0.02em]">{value}</div>
            </div>
          ))}
        </section>
        ...
        <section className="mb-4 overflow-hidden rounded-[18px] border border-white/[0.085] bg-white/[0.028] shadow-[0_18px_45px_rgba(0,0,0,0.14)]">
          ...
          <div className="p-4">
            {hasTrendData ? (
              ...
              <div className="flex min-h-[150px] items-center justify-center rounded-xl border border-dashed border-white/[0.09] bg-black/10 px-6 text-center">
                <div>
                  <div className="mx-auto mb-2 flex h-9 w-9 items-center justify-center rounded-full border border-blue-400/15 bg-blue-500/[0.07] text-blue-300">↗</div>
                  <p className="text-sm font-medium text-slate-300">Henüz fiyat trendi oluşmadı</p>
                  <p className="mt-1 text-xs text-slate-500">Trend göstermek için en az iki fiyat kaydı gerekir.</p>
                </div>
              </div>
            )}
          </div>
        ...
        <section className="overflow-hidden rounded-[18px] border border-white/[0.085] bg-white/[0.028] shadow-[0_18px_45px_rgba(0,0,0,0.13)]">
            ...
            <table className="min-w-full text-xs">
              ...
              <tbody>
                {history.map((item, index) => {
                  ...
                  return (
                    <tr key={`${item.sku}-${item.checkedAt}-${index}`} className="border-t border-white/[0.07] transition hover:bg-white/[0.018]">
